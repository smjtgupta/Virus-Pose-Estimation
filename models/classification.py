...
checkpoint_file = 'class_model.hdf5'
...
# assume matfiles of 2048 images
    ...
    # imgs (256,256,2048,22)
    return imgs

def gen_data(imgs,num_pairs):
    ...
    for i in range(num_rand_pairs):
        ...
        img_pairs[i,0] = cv2.resize(imgs[:,:,a,c],(img_size,img_size))/255.0
        img_pairs[i,1] = cv2.resize(imgs[:,:,b,d],(img_size,img_size))/255.0
        img_label[i] = 1 * ( c == d )
...
if __name__ == '__main__':
    ...
    for i in range(1): # num times to drop learning rate
        ...
        for j in range(10): # num times to generate data ~8M images
            [img_pairs, img_label] = gen_data(imgs, num_pairs)
            ...
            #scores = model.evaluate([input_top, input_bot], [img_label], verbose=0)
            #print(scores.dtype)
