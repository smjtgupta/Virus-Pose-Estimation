...
def cos_distance(y_true, y_pred):
    ...
    return 100*K.mean(tf.acos(K.abs(y_true * y_pred)), axis=-1)
...
if __name__ == '__main__':
    ...
    conv7 = Convolution1D(32, 3, border_mode = 'same', activation='relu')(conv6)
    conv7 = Convolution1D(32, 3, border_mode = 'same', activation='relu')(conv7)
    pool7 = MaxPooling1D(pool_length=2)(conv7)
    
    conv8 = Convolution1D(32, 3, border_mode = 'same', activation='relu')(conv7)
    conv8 = Convolution1D(32, 3, border_mode = 'same', activation='relu')(conv8)
    pool8 = MaxPooling1D(pool_length=2)(conv8)
    
    conv9 = Convolution1D(32, 3, border_mode = 'same', activation='relu')(conv8)
    conv9 = Convolution1D(32, 3, border_mode = 'same', activation='relu')(conv9)
    ...
    fc1 = Dense(64, activation='relu', name='fc1')(pool9)
    ...
    rms = RMSprop()
    model.compile(loss=cos_distance, optimizer=rms)
    ...
    for i in range(2): # num times to drop learning rate
            ...
            for j in range(2): # num times to generate data ~8M images
                print("i,j={0},{1}".format(i,j))
                model.fit([input_top, input_bot], [img_rot], validation_split = 0.1, batch_size=128, nb_epoch=nb_epoch, callbacks=[model_checkpoint])
