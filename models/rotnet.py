...
def gen_data(imgs,quats,num_pairs):
    print('Generating {0} pairs'.format(num_pairs))
    tic = time()
    ...
    for i in range(num_rand_pairs):
        ...
        img_pairs[i,0] = (imgs[:,:,a,c])
        img_pairs[i,1] = (imgs[:,:,b,d])
        img_label[i] = 1 * ( c == d )
    ...
    for i in range(num_same_pairs):
        ...
        c = same_pairs_set[i]
        #img_pairs[num_rand_pairs + i,0] = np.random.uniform(0.0,1.0,(256,256))
        #img_pairs[num_rand_pairs + i,1] = np.random.uniform(0.0,1.0,(256,256))
        img_pairs[num_rand_pairs + i,0] = cv2.resize(imgs[:,:,a,c],(img_size,img_size))/255.0
        img_pairs[num_rand_pairs + i,1] = cv2.resize(imgs[:,:,b,c],(img_size,img_size))/255.0
        img_label[num_rand_pairs + i] = 1
        img_rot[num_rand_pairs + i] = img_label[i] * quatmultiply(quats[b,:,c], quatinv(quats[a,:,c]) )

    print('Took {0} sec to generate data'.format(time()-tic))
...
if __name__ == '__main__':
    ...
    flat_dim = (img_size*img_size*256/(8*8),) #change the number
    vec_dim = (512,)            
    # network definition
    ...
    featureTopExpand = Lambda(expand_dims,name='expand_top')(feat_a)
    featureBotExpand = Lambda(expand_dims,name='expand_bot')(feat_b)
    #custom_corr = merge([feat_a, feat_b], mode='concat', concat_axis=1)
    custom_corr = merge([featureTopExpand, featureBotExpand], mode='concat', concat_axis=2)
    
    #crm layers
    
    conv6 = Convolution1D(16, 3, input_shape = vec_dim, border_mode = 'same', activation='relu')(custom_corr)
    conv6 = Convolution1D(16, 3, border_mode = 'same', activation='relu')(conv6)
    pool6 = MaxPooling1D(pool_length=2)(conv6)
    
    conv7 = Convolution1D(32, 3, border_mode = 'same', activation='relu')(pool6)
    conv7 = Convolution1D(32, 3, border_mode = 'same', activation='relu')(conv7)
    pool7 = MaxPooling1D(pool_length=2)(conv7)

    conv8 = Convolution1D(64, 3, border_mode = 'same', activation='relu')(pool7)
    conv8 = Convolution1D(64, 3, border_mode = 'same', activation='relu')(conv8)
    pool8 = MaxPooling1D(pool_length=2)(conv8)
    
    conv9 = Convolution1D(128, 3, border_mode = 'same', activation='relu')(pool8)
    conv9 = Convolution1D(128, 3, border_mode = 'same', activation='relu')(conv9)
    pool9 = MaxPooling1D(pool_length=2)(conv9)

    conv10 = Convolution1D(128, 3, border_mode = 'same', activation='relu')(pool9)
    conv10 = Convolution1D(128, 3, border_mode = 'same', activation='relu')(conv10)
    pool10 = MaxPooling1D(pool_length=2)(conv10)
    
    pool10 = Flatten(name='flatten')(pool10)
    
    # keeping fc small for now to keep num parameters small
    # camera pose estimation layer
    fc1 = Dense(128, activation='relu', name='fc1')(pool10)
    fc2 = Dense(128, activation='relu', name='fc2')(fc1)
    
    #classification = Lambda(euclidean_distance, output_shape=eucl_dist_output_shape)([processed_a, processed_b])
