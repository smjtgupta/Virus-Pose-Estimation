...
input_dim = (28,28,1)
flat_dim = (3136,) #change the number
nb_epoch = 1
num_outputs = 4
num_pairs = 1500 #>1000
...
# crm layers
conv6 = Convolution1D(32, 3, border_mode = 'same', activation='relu')(custom_corr)
conv6 = Convolution1D(32, 3, border_mode = 'same', activation='relu')(conv6)
pool6 = MaxPooling1D(pool_length=2)(conv6)
#
conv7 = Convolution1D(64, 3, border_mode = 'same', activation='relu')(conv6)
conv7 = Convolution1D(64, 3, border_mode = 'same', activation='relu')(conv7)
pool7 = MaxPooling1D(pool_length=2)(conv7)
#
conv8 = Convolution1D(128, 3, border_mode = 'same', activation='relu')(conv7)
conv8 = Convolution1D(128, 3, border_mode = 'same', activation='relu')(conv8)
pool8 = MaxPooling1D(pool_length=2)(conv8)
#
conv9 = Convolution1D(128, 3, border_mode = 'same', activation='relu')(conv8)
...
fc1 = Dense(128, activation='relu', name='fc1')(pool9)
...
# train
rms = RMSprop()
model.compile(loss=[contrastive_loss,'mse'],  loss_weights=[0.0, 10.0], optimizer=rms)
...
for i in range(1): # num times to drop learning rate
        ...
        for j in range(10): # num times to generate data ~8M images
            [img_pairs, img_rot, img_label] = gen_data(imgs, quats, num_pairs)
            input_top = np.expand_dims(img_pairs[:,0], axis=3)
            input_bot = np.expand_dims(img_pairs[:,1], axis=3)
            model.fit([input_top, input_bot], [img_label, img_rot],
            validation_split = 0.1, batch_size=128, nb_epoch=nb_epoch)
